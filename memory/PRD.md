...
### Iteration 13 (2026-02-21): Image hardening + SEO + Templates + Cart Recovery + POS Barcode + Header/Footer + Receipt
- **Bug fix**: Cart cleared too early during PayHere checkout → customer briefly saw "Cart Empty" before browser navigated to PayHere. Now the hidden form submits FIRST and the clear happens on a microtask. Cart provider also re-hydrates synchronously from localStorage to survive reloads.
- **Image upload pipeline**: `/app/frontend/src/lib/image.js` auto-resizes (max 2400px) + JPEG-compresses (~85%) on the client. Per-context limits — products 1.5MB, page-builder backgrounds 3MB, logos 1MB. Upload UI shows final size + dimensions hint. Products `uploadImage` auto-retries with aggressive 700KB re-compress on 413, then surfaces a clear "ask your host to set `client_max_body_size 5M;`" toast. Backend rejects >4MB base64 with HTTP 413.
- **SEO**: `/api/sitemap.xml` (also `/sitemap.xml` for production Nginx) dynamic from active products + categories + published custom pages. In-memory 5-min cache so Googlebot doesn't hammer Postgres. `/api/robots.txt` blocks `/admin /api /checkout /account` and declares the sitemap.
- **Prebuilt notification templates**: `templates_seed.py` ships 15 professionally-worded templates (email + SMS for order_placed/paid/shipped/delivered/cancelled + abandoned_cart + marketing_promo + winback). Branded HTML version (`body_html`) for email events. Idempotent installer at `POST /api/admin/templates-library/install`. Dispatcher now sends multipart `text+html` when a template has `body_html`.
- **Cart abandonment recovery**: new `CartSession` model + `POST /api/cart/sync` (storefront pings on every checkout-form change). Background asyncio worker every 60s dispatches `abandoned_cart` template if enabled (configurable wait time + channels). Admin UI at Marketing → Abandoned Carts: enable toggle, wait-time slider, channel pills, recent sessions list, "Run worker now" trigger. Order placement marks open sessions converted (de-dup by email/phone).
- **Header & Footer presets**: `header_layout` (classic | centered | split), `footer_layout` (columns | minimal | brand), plus per-element bg/text/hover colors. Settings → Header/Footer tab. Color CSS variable injected so hover state respects the chosen color across the storefront.
- **Per-section background image**: every page-builder section now accepts `bg_image_id` (or URL), size (cover/contain/stretch), and position. MediaUploader is reused with `maxMB=3` cap.
- **Customisable receipt template**: 80mm / 58mm / A4 sizes (CSS @page rules), custom header/footer text with `{{brand_name}}` placeholders, toggles for logo / QR / order barcode / tax line. Configured at Settings → Receipt; rendered server-side via `/api/receipt/{order_number}`.
- **POS barcode**: new `Variant.barcode` column + lookup endpoint `/api/admin/barcode/lookup/{code}` (falls back to sku). Auto-focused scan input in POS adds product to cart instantly when a USB scanner types code + Enter. Admin → Products has a "Print labels" button → `/admin/barcode-labels` (JsBarcode-rendered Code128 sheet, configurable 2-6 labels per row).
- **Customer opt-in flags**: `marketing_opt_in / email_opt_in / sms_opt_in` columns on `customers` (default true). Bulk send honours per-channel flag when `marketing_opt_in_only=true`.
